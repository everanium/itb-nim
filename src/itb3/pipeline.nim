...
type
  PipeHandle = object
    ...
    ## releases it (libitb3 closes and zeroes key material first).
    h: csize_t
...
func rawHandle*(p: Pipeline): csize_t =
  ## The raw libitb3 handle. Internal plumbing for the stream module —
...
proc initPipeline*(profile: string, opts = Opts()): Pipeline =
  ...
  ## attempt is closed by libitb3 before returning).
...
proc loadPipeline*(blob: openArray[byte],
  ...
  ## libitb3). The profile shape travels inside the blob — no profile
...
proc loadPipelineF*(path: string,
  ...
  ## inside libitb3 (a missing or unreadable file fails with
...
proc cipherCallInto(p: Pipeline,
  ...
  if int(n) > dst.len:
    ...
    raise newException(ValueError,
      "libitb3 reported " & $int(n) & " output bytes for a " &
      $dst.len & "-byte buffer")
  int(n)
