## Runtime symbol binding over the libitb3 shared library.
##
...
## ``could not load: libitb3.so`` diagnostic at program start.
##
## Every prototype mirrors ``cmd/cshared``'s generated ``libitb3.h``;
...
const
  LibFilename =
    when defined(windows): "libitb3.dll"
    elif defined(macosx): "libitb3.dylib"
    else: "libitb3.so"
  DistSubdir =
...
proc itbLibPath*(): string =
  ...
  let env = getEnv("ITB_LIBITB3_PATH")
  if env.len > 0:
    cachedLibPath = env
    return cachedLibPath
  var dir = getAppDir()
