...
# libitb.so via build.sh, then compiles and runs bench/bench.nim:
...
nim c -d:release --hints:off --outdir:bench/build \
    --nimcache:bench/build/nimcache bench/bench.nim

# bench/build/bench reads ITB_MSG_PROFILE for the Message shape and
