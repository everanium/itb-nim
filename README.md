...
Thin Nim proxy over the libitb3 shared library's `ITB_Triple_*`
surface (`cmd/cshared`). Runtime FFI via `{.importc, dynlib.}`
declarations — no compile-time link against libitb3; the
...
tested against Nim 2.2). Windows: the same; libitb3 builds as
`libitb3.dll`. No third-party Nim packages are required — the binding
uses only the standard library.

## Build

The convenience driver builds `libitb3.so`, type-checks the Nim
...
The harness builds `libitb3.so`, exports `ITB_LIBITB3_PATH`, and runs
...
- Blocking FFI calls (`write` on a saturated session, `read` after
  ...
  runs.
- libitb3 must be reachable at program start through the lookup order
  above; a resolve failure surfaces as the Nim runtime's
  `could not load: libitb3.so` diagnostic and terminates the process.
